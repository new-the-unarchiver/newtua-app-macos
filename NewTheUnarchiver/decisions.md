...
- ~~На скриншоте в шапке видно «24 bytes» (английский) даже когда
  ...
  extension'ам locale).~~ **2026-06-24 опровергнуто пользователем
  на пре-альфа-прогоне:** при ручной проверке Release-сборки из
  `/Applications/` русская локаль в Quick Look extension
  подхватывается штатно, `ByteCountFormatStyle(locale:
  Locale.current)` форматирует размер на русском. Никакого
  ограничения Apple на `Locale.current` в QL extension нет.
  Исходный диагноз был ошибкой автора — скриншот был снят на
  Debug-сборке до коммита `d1624bd`, в котором локализация
  ещё не была дописана. Workaround через UserDefaults shared
  group **не нужен**.
...
- ~~Локализация Quick Look extension не подхватывает русский даже на
  ...
  контексте.~~ **2026-06-24 опровергнуто пре-альфа-прогоном:** русская
  локаль подхватывается штатно, форматирование размера и плюралы на
  русском работают. Никакого ограничения нет — см. секцию ниже за
  ту же дату («опровержение»).
...
## 2026-06-24 — Регистрация QL extension после установки в /Applications

При первом ручном прогоне Release-сборки выяснилось, что **`lsregister
-f` не работает** для регистрации Quick Look extension в нашем
бандле — Spotlight отвергает скан `.appex` с ошибкой `-10811`, и
плагин не попадает в реестр Plugin Kit Daemon. Симптом: после
копирования `NewTheUnarchiver.app` в `/Applications/` команда
`pluginkit -m -i <bundle-id>` возвращает пусто, даже если перед этим
прогнать `lsregister -f -v /Applications/NewTheUnarchiver.app`.

Параллельно есть вторая ловушка: pluginkit предпочитает
DerivedData-копии (Debug и Release из `~/Library/Developer/Xcode/`,
плюс локальный `-derivedDataPath build/`), даже если они уже не
актуальны. Они «забивают» bundle-id и `Path` показывает их, а не
`/Applications/`.

**Рабочая последовательность для активации QL extension:**

```bash
# 1. Снести все конкурирующие регистрации (если были запуски из Xcode):
...
$LSR -u ~/Library/Developer/Xcode/DerivedData/NewTheUnarchiver-*/Build/Products/Debug/NewTheUnarchiver.app
$LSR -u ~/Library/Developer/Xcode/DerivedData/NewTheUnarchiver-*/Build/Products/Release/NewTheUnarchiver.app
$LSR -u ./build/Build/Products/Release/NewTheUnarchiver.app  # если был ручной xcodebuild

# 2. Прямая регистрация через pluginkit (в обход Spotlight):
pluginkit -a /Applications/NewTheUnarchiver.app/Contents/PlugIns/NewTheUnarchiverQuickLook.appex
pluginkit -e use -i aleksei.trankov.NewTheUnarchiver.NewTheUnarchiverQuickLook

# 3. Перезапуск QL-демона и Finder:
qlmanage -r
qlmanage -r cache
killall quicklookd
killall Finder
...
# 4. Проверка:
pluginkit -mvv -i aleksei.trankov.NewTheUnarchiver.NewTheUnarchiverQuickLook
# должно показать "+ ..." и Path = /Applications/NewTheUnarchiver.app/...
```

**Bundle ID extension** — `aleksei.trankov.NewTheUnarchiver.NewTheUnarchiverQuickLook`
(собран Xcode из dev-team `5FVC5YT2B5`, identity
`Apple Development: trankov@ya.ru`).

**Почему так:** `lsregister -f` идёт через путь Spotlight indexing,
который на этой машине отказывает для extension bundles. `pluginkit
-a` обращается напрямую к pkd через XPC, минуя индексацию. Это
задокументированный способ, хоть и менее известный.

Подпись (Apple Development identity) и entitlements (`app-sandbox` +
`files.user-selected.read-only` у extension, отсутствие sandbox у
app) — правильные, проблема была чисто в регистрации, не в правах
доступа.

**Что записано в инструкции пользователю:** обновлённый порядок
команд для активации после установки в `/Applications/` — см.
этот чат-лог. В отдельный markdown-документ выносить не нужно: это
одноразовая ручная операция при инсталляции пре-альфы без
notarization.

## 2026-06-27 — Карта архитектуры кода (`ARCHITECTURE.md`)

Создан `ARCHITECTURE.md` — карта текущего устройства кода: слои
(Domain / Engine / Views / Settings / QuickLook + extension), что
делает каждый файл, граф взаимодействий, модель конкурентности,
известные ограничения v1. Заполняет пробел: `decisions.md` отвечает на
«почему», `plan.md` — на «что делали по этапам», а карты «как устроено
сейчас» не было. Ссылка добавлена в раздел «Связанные документы»
`CLAUDE.md`. Источник истины — код; при расхождении правим карту и
фиксируем здесь.

## 2026-06-27 — Точный детект носителя (IOKit) для `SystemVolumeProbe`: откладываем в v1.1

**Решение:** «сильное» определение типа носителя через IOKit + Disk
Arbitration **сейчас не делаем**. Текущая эвристика «внутренний том =
SSD, внешний/Fusion = unknown (serial)» в `Engine/VolumeProbe.swift`
остаётся как есть.

**Почему:**
- Цель — macOS 26+ / Apple Silicon, где внутренний накопитель **всегда
  SSD**. Эвристика там уже даёт правильный ответ в 100% случаев.
- Единственное, что добавил бы точный детект, — распознавание быстрых
  **внешних** SSD (TB/USB-NVMe) ради параллели на них. Сценарий узкий,
  и выигрыш не гарантирован (внешняя шина часто сама узкое место).
- Стоимость несоразмерна: ~150–250 строк хрупкого IOKit-кода + обход
  слоёв APFS (том → контейнер → store), а главное — проверять можно
  **только на реальном железе** под каждый тип носителя (на CI — никак).
- Безопасный fallback на serial ничего не ломает; цена текущей ошибки —
  лишь иногда избыточная последовательность.

**Когда вернуться:** при реальных жалобах пользователей на медленную
последовательную распаковку с внешних SSD — тогда выгода измерима и
есть сценарий для проверки.

**Полная аналитическая справка** (стоимость / сложность / целесообразность,
по осям) — в `analysis-2026-06-27-volume-probe-iokit.md`. Этот же пункт
учтён в разделе «Известные ограничения v1» в `ARCHITECTURE.md`.


## 2026-06-27 — Поддержка LZ4 в GUI: новый пре-альфа тестовый релиз

**Контекст.** После прошлого тестового релиза (v1) в Rust-ядро добавили
формат **LZ4** (`.lz4` / `.tar.lz4`, frame-формат). Список объявляемых
системе форматов в приложении к этому моменту уже был расширен соседним
агентом с «популярного» поднабора до полного набра ядра (zst, lzma, z,
zipx, cab, deb, rpm, cpio, xar, pkg, ar, msi, iso, warc, exe), но **LZ4
в нём не хватало** — единственный формат ядра, не объявленный в GUI.
Соседний агент остановился на прогоне тестов (средовой сбой xcode-mcp),
не доведя до зелёного гейта и релиза.

**Что сделано (только GUI-слой, движок не трогали):**
- `Engine/SupportedFormats.swift` — добавлен
  `Format(utiIdentifier: "public.lz4-archive", extensions: ["lz4", "tar.lz4"])`
  в группу компрессоров (после `public.z-archive`). Всего форматов: 22 → **23**.
- `NewTheUnarchiver/Info.plist` — новый `CFBundleDocumentTypes` для
  `public.lz4-archive` (меню Finder «Открыть в программе» / двойной клик).
- `NewTheUnarchiverQuickLook/Info.plist` — `public.lz4-archive` добавлен
  в `QLSupportedContentTypes` (превью Quick Look для `.lz4`).
- `Localizable.xcstrings` — ключ `format.lz4.name` (EN «LZ4 archive»,
  RU «Архив LZ4»). 23 ключа `format.*`, TODO нет.
- Тесты `Stage5Tests`/`Stage7Tests` — счётчик 22 → 23, добавлены
  `lz4`/`tar.lz4` и `public.lz4-archive` в проверяемые наборы.

**Ключевое техническое решение — системный UTI, не импортированный.**
Проверка показала: `UTType(filenameExtension: "lz4")` и
`UTType("public.lz4-archive")` разрешаются в **системный** тип
(не dynamic) на macOS 26. Поэтому, в отличие от `ar`/`msi` (у которых
нет системного UTI и заведены `UTImportedTypeDeclarations`), для LZ4
объявлять собственный UTI **не нужно** — используем `public.lz4-archive`
напрямую. Меньше деклараций, надёжнее резолвинг в тестовой среде.

**Гейт.** Полный юнит-таргет `NewTheUnarchiverTests` — **248 passed,
0 failed, 0 skipped** (серийно, с тайм-аутами по политике CLAUDE.md).
Форматные сюиты (Stage5/Stage7/Stage7Extended) зелёные и до, и после
правок.

**Релиз.** Release-сборка собрана в
`build/Build/Products/Release/NewTheUnarchiver.app` (5,3 МБ, подпись
Apple Development, как и раньше). В бандле проверено наличие
`public.lz4-archive` и в `Info.plist` приложения, и в `Info.plist`
QuickLook-расширения. Доставка — как у прошлой пре-альфы: копия в
`/Applications/` + ручная регистрация (`pluginkit` / `qlmanage` /
`killall Finder`) по рецепту из записи «2026-06-24 — Регистрация QL
extension». DMG/notarization для пре-альфы не делаем.

**`project.pbxproj` не трогали** — Xcode был открыт; новых файлов в
таргеты не добавляли (правки только в уже включённых ресурсах и
исходниках), поэтому инструкция по pbxproj не требовалась.

## 2026-06-27 — Установочный скрипт пре-альфы (`tools/install-prealpha.sh`)

**Контекст.** Раньше установка пре-альфы (копия в `/Applications` +
регистрация Launch Services и Quick Look) была ручным набором команд в
чат-логе (см. запись «2026-06-24 — Регистрация QL extension», там было
решено скрипт не заводить). На практике при выпуске LZ4-релиза набор
команд проверили вживую и закрепили скриптом — ручной список устаревает
и легко ошибиться при повторе.

**Файл:** `apps/macos/tools/install-prealpha.sh` (рядом с
`build-newtua-xcframework.sh`, тот же стиль: `set -euo pipefail`,
`REPO_ROOT` через git, явный `PATH`).

**Что делает:**
1. Собирает Release (`xcodebuild -derivedDataPath build`); `--no-build`
   переиспользует готовый бандл.
2. Копирует `.app` в `/Applications` (путь меняется через `DEST=`).
3. Снимает конкурирующие регистрации из DerivedData и `./build`
   (ошибки сканирования Spotlight `-10811/-10814` ожидаемы и глушатся).
4. `lsregister -f` для UTI/типов документов приложения.
5. `pluginkit -a` + `pluginkit -e use` для Quick Look-расширения
   (напрямую через pkd, в обход Spotlight — `lsregister -f` для `.appex`
   на этой машине не работает).
6. Перезапускает `quicklookd` и Finder.
7. **Проверяет:** расширение включено (`pluginkit` показывает `+` и путь
   в `/Applications`) и `.lz4` опознаётся как `public.lz4-archive`.

Bundle id расширения скрипт читает из самого `.appex`
(`CFBundleIdentifier`), а не хардкодит — не разъедется с настройками
подписи.

**Проверено вживую (2026-06-27):** `--no-build` прогон → exit 0, обе
проверки OK; меню Finder «Открыть в программе» и превью Quick Look
по Space показывают `.lz4` наравне с остальными форматами.

**Граблю с многобайтовым `…` в строках echo** убрали — под `set -u`
bash спотыкался на `$VAR…` (переменная вплотную к UTF-8-символу).
В скрипте только ASCII `...`.

Запись «2026-06-24 — выносить в отдельный документ не нужно» этим
**отменяется**: скрипт удобнее одноразового списка и теперь и есть
канонический способ установки пре-альфы.

## 2026-06-27 — Скилл «add-macos-format» (полный цикл добавления формата)

**Контекст.** Перенос нового формата из ядра в приложение — повторяющаяся
процедура (последний раз — LZ4). Чтобы будущий ассистент не выводил шаги
заново и не упускал точки (Quick Look, локализация, два хардкод-счётчика в
тестах, стратегия UTI), закрепили процедуру скиллом.

**Файл:** `apps/macos/.claude/skills/add-macos-format/SKILL.md`
(в зоне macOS-приложения, рядом с его `.claude`-настройками, чтобы
подхватывался в сессиях с рабочей папкой `apps/macos/...`).

**Что описывает (полный цикл, как делали для LZ4):**
0. Разведка: ветка/чистота, чтение decisions/CLAUDE/ARCHITECTURE, **дельта
   «ядро vs приложение»** (`FormatId`/`Compressor`/срез расширений в `detect.rs`
   против `SupportedFormats.swift`), проверка, открыт ли Xcode.
1. Выбор стратегии UTI: системный (`UTType(filenameExtension:)`,
   `dynamic=false` — как `public.lz4-archive`) → использовать напрямую; иначе
   собственный импортированный UTI (как `ar`/`msi`).
2. Пять синхронных правок: `SupportedFormats.swift`, `Info.plist`
   (`CFBundleDocumentTypes` + при нужде `UTImportedTypeDeclarations`),
   QuickLook `Info.plist` (`QLSupportedContentTypes`), `Localizable.xcstrings`
   (`format.<ext>.name`, RU+EN), тесты Stage5/Stage7 (два счётчика + списки).
3. Гейт: `build-for-testing` + серийный `test` с тайм-аутами до
   `failedTests = 0`.
4. Сборка+установка через `tools/install-prealpha.sh` (с подтверждением —
   трогает систему).
5. Проверка: `pluginkit` `+`/путь, резолв расширения, паритет превью с `.zip`.
6. Запись в `decisions.md`.
7. Коммит и слияние с `dev`.

**Границы в скилле зашиты:** `crates/`/Cargo — только чтение; `pbxproj` не
трогать при открытом Xcode; решения — в `decisions.md`; строки — RU+EN.

Указатели на скилл и на `install-prealpha.sh` добавлены в раздел «Связанные
документы» в `CLAUDE.md` приложения.
